...
```
  ...
  The question explores a wider phenomenon observed in other competitive industries, most notably eSports. In the early stages of games such as *Dota 2*, professional structures were still developing, and organizations lacked the resources, infrastructure, and support networks that are now commonplace. As a result, individual superstars were able to dominate the competitive landscape, often carrying teams to success through sheer skill and talent. However, as the industry matured, teams began to invest in systematic practice routines, coaching staff, analysts, and organizational frameworks designed to maximize team performance. With this shift, the importance of a single “superstar” diminished relative to the strength of the collective unit, and success increasingly depended on cohesion, role specialization, and coordinated strategy, leading to more balanced salaries within the same organization. 

  This historical precedence raises questions on whether basketball underwent a similar evolution and whether eras have shifted the relationship between salary inequality and team performance in the NBA.
...
## Implementing the Gini coefficient to quantify Salary Distribution
...
```{r, include = FALSE}
# Merging Team Wins and Revenue to Team Gini dataset
...
gini.wins.2000to2015 %>%
  select(team, seasonstart, gini, record, winpercent, teamsalary, averageteamsalary, revenue ) %>% 
    mutate(
    teamsalary = scales::label_number(big.mark = ",", accuracy = 1)(teamsalary),
    averageteamsalary = scales::label_number(big.mark = ",", accuracy = 1)(averageteamsalary),
    revenue = scales::label_number(big.mark = ",", accuracy = 1)(revenue)
  ) %>%
...
It is observed that the Los Angeles Lakers have the second highest average revenue, with the team being an interesting candidate to study due to their strong regular and postseason success in this time period along with multiple superteams (both the Shaq-Kobe, Kobe-Gasol dynasties). This study can seek to answer the question whether building superteams around Kobe with multiple superstars benefitted the team success of the Lakers. 
...
```{r}
## Regression of LA Lakers and Gini Coefficient, Holding Monetary Factors Constant
reg.2000to2012.lakers.monetary <- lm(winpercent ~ gini + teamsalary + revenue, data=gini.wins.2000to2012.lakers)
```

```{r, fig.height=2}
export_summs(reg.2000to2012.lakers, reg.2000to2012.lakers.monetary,
...
```{r}
reg.2000to2015.monetary <- lm(winpercent ~ gini + teamsalary + revenue, data=gini.wins.2000to2015)
```

```{r}
export_summs(reg.2000to2015, reg.2000to2015.monetary, 
...
export_summs(reg.2000to2015, reg.2000to2015.monetary, reg.2000to2012.lakers, reg.2000to2012.lakers.monetary,
