...
ggplot(data = nba.salaries.team, aes(x = seasonstart, y = averagetotalteamsalary)) +
  ...
  ggtitle("Average Team Salary over time (1984 to 2017)")
```
...
It is observed that salaries that are below the minimum salary for each respective season. For example, the sdecond player on this list, "jonesda02", is 12-year NBA veteran Dahntay Jones, who only has a salary of only $8,819 for the 2015 season on the Cleveland Cavaliers. Per his basketball reference page (https://www.basketball-reference.com/players/j/jonesda02.html), it seems as if he only appeared once for the Cavs. After researching further, it's realized that he signed with the Cavs before the last game of the regular season.

Using this example, it is fair to assume that there are many irregular signings that don't last the whole year, which will cause major deviations in salary distribution for teams. While calculating salary distribution by using the Gini coefficient, it is imperative to eliminate these deviations. 

To account this, a salary of floor of $300,000 is enforced (which is around the rookie minimum contract in the year 2000). Found at: https://www.spotrac.com/nba/cba/minimum 
...
ggplot(data = gini.wins.2000to2015, aes(x = seasonstart, y = gini, color = team)) +
  ...
  ggtitle("Gini Salary Coefficient over Time (2000 to 2015)")
```
...
```{r}
ggplot(data = gini.wins.2000to2015.heat, aes(x = gini, y = winpercent, color = seasonstart)) +
  geom_point(size = 2) +
  geom_smooth(method="lm", se = TRUE, color="black") +
  ggtitle("Miami Heat Gini Salary Coefficient vs. Win Percentage (2000 to 2015)")
```
...
summ(reg.2000to2015.heat, digits=5)
```
...
summ(reg.2000to2015.heat.salary, digits=5)
```
...
ggplot(data = gini.wins.2000to2015, aes(x = gini, y = winpercent, color = seasonstart)) +
  geom_point(size = 2) +
  geom_smooth(method="lm", se = TRUE, color="black") +
  ggtitle("Gini Salary Coefficient vs. Win Percentage (2000 to 2015)")
```
...
summ(reg.2000to2015.salary, digits=5)
```
